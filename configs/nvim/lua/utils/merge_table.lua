local function merge_table(destination, source)
    if type(destination) ~= 'table' then error('You should specify the destination as a table', 2) end
    if type(source) ~= 'table' then error('You should specify the source as a table', 2) end
